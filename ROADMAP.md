...
- ~~Integration with ModelGraph (constraints are just edges)~~ Done (`TCAD-CON-5`): constraints are now `NodeType.CONSTRAINT` graph nodes with dependency edges to every part they reference, so `IncrementalBuilder`'s existing invalidation machinery marks a constraint dirty exactly when a part it mates changes. `watcher.py::_rebuild` uses this to skip re-solving constraints entirely when nothing relevant changed, rather than re-solving all of them every rebuild. Deliberately coarse (skip-the-whole-batch, not per-constraint) since the constraints themselves still solve jointly in one `Assembly.solve()` call — per-constraint partitioning would touch that shared/contradiction-checked solve path for a benefit no one has asked for. Investigating this originally (2026-07-19) surfaced a sharper bug fixed the same session: `tiacad watch` was silently skipping `constraints:` entirely (`IncrementalBuilder.build()` had no `constraints_spec` parameter, and `watcher.py` never called `ConstraintBuilder`) — a constrained model built correctly outside watch mode but exported every constrained part at its raw, unconstrained position under `tiacad watch`. See KNOWN_LIMITATIONS.md #1.

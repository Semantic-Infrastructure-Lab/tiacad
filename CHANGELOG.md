...
`GeometryBackend`" ask — `MockBackend`'s boolean/transform ops are
bookkeeping stubs, so no amount of routing through it would have produced a
genuine second kernel. Added `manifold3d` (independent mesh-based CSG
kernel, no code shared with OCCT — also the engine modern OpenSCAD itself
...
Implements `docs/developer/VALIDATION_STRENGTHENING.md` §4.7/§4.9, the two
remaining items from the validation-strengthening plan (the T0–T5 confidence
ladder itself was already complete). Golden STEP topology gate
(`tiacad_core/testing/golden_step.py`): BREP solid/face/edge/vertex-count
signatures for 5 anchor scenarios, computed live and via STEP re-import, to
catch topology drift a volume/bbox check can't see. Trust-gallery sign-off
ledger (`examples/trust/SIGNOFF.md`, generated by `scripts/trust_signoff.py`)
that never overwrites a human's Reviewed/Reviewer/Note once filled in. The
visual review this shipped caught a real bug: `pcb_standoff_assembly.yaml`'s
plate/pcb parts had `height`/`depth` swapped, rendering as a vertical wall
instead of a flat plate — undetectable by a volume-only check, since
swapping which axis is "thickness" doesn't change the volume. Fixed;
documented as `KNOWN_LIMITATIONS.md` #13. `expect:` contracts added to 16 of
24 trust scenarios; `test_trust_contracts.py` trimmed from ~793 to ~330
lines, removing assertions now redundant with those contracts.

...
| `hull_spheres` | PASS | 2026-07-18 | TIA (AI review) | Rounded triangular blob, no flat gaps between spheres — matches description. Render shows faceted (non-smooth) shading, consistent with the "Input points appear coplanar" warning this model already triggers at build time (sphere_c's Y translate is 34, not the equilateral-exact 34.6) — cosmetic tessellation artifact, not a topology defect (watertight, 1 solid); not fixed this session, worth a follow-up tt item if picked up. |
